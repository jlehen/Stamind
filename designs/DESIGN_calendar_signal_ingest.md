...
The tag remains the right choice over a title convention for the other three
reasons; the privacy argument is a bonus that only holds on the full pull.
...
a cancelled private appointment on the unfiltered incremental stream (§3, §6).
...
path it is the only filter there is, and it is what keeps untagged events out of
the database (§3). The full pull relies on the server-side filter and the guard
is a no-op there.
