...
equipment decide the content, and "nothing else". And it sized a session by adding up the
2 to 3 minute rests the athlete's own science file asks for, where this athlete takes turns
on two exercises, rests as long, and fits twice as much in the hour. Now the
...
said in the session's notes, so no table changes. A session whose brief the week planner
wrote anew is written again too (§9), which is how one `workout generate` brings the 24
sessions already on the calendar to the new shape (§11.3).
...
set is never the set that missed (§8, §10). A held load no longer comes "with a note", which
a kept session had nowhere to put (§10). A restore leaves the session's `strength_checks`
row alone (§9). And the layout is repaired: the third source of names in §7 and the open
questions of §12 are list items again, and the long lines are wrapped.
...
arrives with its lineage keeps them, which a `replace` in `workout generate` and — since
rev. 17 — a move in `workout adapt` both do (§9).

Revision 13 takes the first fixes from a review of revision 12. `prescribed_sets` and
`strength_checks` are schema 16, since main reached 15 in the meantime. And "brief", a word
this design brought in and used without saying so, joins the names below.
...
planner revised or moved keeps its sets unless the brief or the evidence changed (§9). A
strength planner call that fails twice fails the proposal, as a failed week planner call
does, so a session with a brief and no sets never exists, and
...
history puts "prescribed" in one place, on the day, and lists the accessories like any other
exercise (§8); the strength planner is shown each exercise's equipment class, which §4
promised it and §9 forgot (§9); the reason for changing a kept session lands in the
revision's reason column, so the paths without a preview keep it (§9); a kept session
changes only on new evidence, a stamp the history's writers bump, which also decides when
the strength planner runs (§9); the shipped science
...
2. **The athlete, through the queue.** Two questions, both queue kinds, neither asked on
   ...
   **The first question, kind `sets_final`, asks whether the sets in Garmin are final.**
   ...
   from Garmin, freezes them, makes the guesses still standing the athlete's, and queues the
   naming questions for what is still unnamed; when Garmin cannot be reached the answer is
   not applied and the question stays waiting (§11). "No, leave it", the drop, freezes the
   sets as they were first read and asks no names: the guesses stay guesses, the unnamed
   groups stay unnamed, all of them count as volume, and `strength name` can still name
   them. Its label says what it costs, because "leave it as it is" and "yes, final" both
   read as "it's fine" at breakfast, and the wrong tap would keep six names out of the
   history for good; on an activity with no guesses it reads "no, leave it unnamed". "Not
   now", the queue's own, is "no, I'll fix them in Connect, ask me again": in 1 hour, in 1
   day or after the others. The check calls the question stale when the activity was frozen
   by other means (`strength reset`, or `strength name`, which freezes an activity it names,
   §11.1), discarded, or is gone. The companion wording: "Thursday's 09:43 gym session has 6
   exercises the watch only guessed and 5 groups of sets it couldn't name. Are the sets in
   Garmin final?" Both wordings say "gym session", because that is what an athlete calls a
   visit to the gym; the design's own word for the thing is activity.
...
counted by day (§3). Each entry then shows that exercise's last three days, discarded ones
skipped again and light ones not counted, however far back they go from `sets_since` on. A
light day (§10) is shown when it falls after the oldest of the three, and is not one of
them: an athlete who lifts three times a week comes out of a light week with three light
days on record, and the day to resume from has to be on the page. The eight days decide
which exercises appear, not how much of each is shown. Accessories are in because the
...
on both sides of the colon. The
...
**What was prescribed and not done is listed too.** An entry shows an exercise only when the
athlete did it, so Monday's chin-ups, prescribed and never lifted, would appear nowhere, and
the strength planner would write them every Monday without knowing. After the entries comes
one line per day, from yesterday back to the oldest of the eight, for each day that had a
strength session with prescribed sets and did not do all of it: the exercises with no named
set that day, with what was prescribed, or the whole session when the day has no strength
activity:
...
```
## SESSIONS AS DONE
The same 8 strength days, each activity as a whole: its length, every set lifted in it, its
RPE, then the exercises in the order they first came. Exercises joined by "+" were
alternated.
...
not, because its job is to say how much this athlete fits in the time. The rests the watch
records between them are not sets and are not counted. The lines under it show named sets
only, as everywhere in the history, and print them the way the entries do.
...
The not-done line and the "(not prescribed)" mark are both made by code, by comparing
names, so the strength planner does not have to find a swap by reading twelve exercises
against six. A day with no prescribed session has no marks: everything in it was the
athlete's own choice. A planned day with no strength activity has no session to show, and
the not-done lines are where it appears.

On a day with two activities, one of them gets the marks: the one that holds the most
prescribed exercises, and the earlier one when they tie. The other is the athlete's own, like
a day with no prescribed session. It is Tuesday September 22. The gym session is prescribed,
and the athlete does it at 09:00, goblet squats included. At 20:10 they do goblet squats and
one-arm rows with a 16 kg kettlebell at home, as on the evening of September 3: 18 minutes
at RPE 2. The morning holds the most prescribed exercises, so it gets the marks. The evening
is not an attempt at the gym session, so its rows are not "(not prescribed)", and two such
evenings do not put rows into the gym day (§9). Code compares names and nothing else. It
does not decide that the evening is a light session or an accessory one. Its head line,
"20:10, 18 min, 12 sets, RPE 2", is what tells the strength planner that it is not
comparable to a 65 minute gym day.
...
pairs. This does not shorten the rests. The athlete's own science file asks for 2 to 3
minutes between sets, the October 1 brief says "long complete rests", and revision 17 read
both as minutes when nothing is lifted: October 5 came out at 18 sets. An athlete who does a
set of belt squats, then a set of push presses, then belt squats again has rested the belt
squat about three minutes. The TASK says so in one sentence, so the science file and the
habit ask for the same session. Where they do disagree, the science file wins, as it does
everywhere (below). Alternated exercises are written one after the other, and the notes say
"alternate the belt squat and the push press, then the hamstring curl and the pulldown". The
rows of `prescribed_sets` do not change. A column that holds the pairs as data is left out
until something reads it, which pushing a session to the watch would (§12).
...
difference counts from the second time it happens in comparable sessions among the days the
sessions as done show. It is Thursday October 1. The session says step-ups, and the athlete
does the leg press. Friday morning the strength planner sees one not-done line and one "(not
prescribed)" mark. That is once, and Thursday October 8 keeps its step-ups. On October 8 the
athlete does the leg press again. Friday morning the strength planner sees the same swap
twice, and the leg press goes into every Thursday left in the mesocycle (below). The same
count applies to an exercise the athlete adds, one they skip, and a number of sets they do
differently: three sets written and four done, twice, means four. An activity with no "(not
prescribed)" mark to get, because the day had no prescribed sets or because the day's other
activity was the attempt at them (§8), has nothing to differ from. Everything in it was the
athlete's choice and counts at once. That is every activity of this athlete's September, all
done before a kilogram was prescribed.
...
prescribed on Thursdays only. Four Thursdays span about ten strength days, the sessions as
done show eight, and by the fourth refusal the first has left them. The comment in the
template says so, and no code guards it. A not-done line can be older than the eight days,
because the not-done lines start where the oldest entry does (§11.2). It has no session
beside it, and it is not counted.
...
so when the strength planner writes it only its title line is kept up there. The kilograms
the athlete reads and the kilograms stored as data cannot disagree, because both are
rendered from the same rows. Monday September 14 after the strength planner, shown the
history of §8 with its accessories and Monday's "full featured gym":
...
The belt squat, the glute bridge, the pulldown and the push press are there because they are
what the history holds for a Monday at the gym, at the loads they hold: September 7's belt
squat, 3×5 at 140, did not reach the top of a 4–6 range, so 140 stands (§10). The
accessories are there at what the athlete last lifted. The chin-up has no load and a
starting point: the history has no chin-up a person named. Wednesday September 16 is at
home, where the profile lists kettlebells up to 24 kg, and the brief reads "moderate
full-body strength at home, 45 min, easy enough to be fresh for Thursday's intervals". The
strength planner writes the goblet squat at 48 kg, the heaviest pair of bells at home, the
one-arm row at 24 kg, and a kettlebell floor press at 2×16 as a starting point, because no
press done at home is in the history.
...
morning the pull reads Monday's sets. Thursday September 17 holds two sessions, the
intervals and the gym, and the gym session, written on Sunday, holds "belt squat 3×4–6 @
140", Monday's numbers, since nothing is written ahead as a ladder (§10). On Tuesday
`workout adapt` runs, and the week planner keeps Thursday. The strength planner is shown
Thursday with its prescribed sets, and the history, which now says "Mon Sep 14, prescribed
1×5 @ 120, 3×4–6 @ 140: 1×5 @ 120, 3×6 @ 140 | RPE 7". It answers with Thursday's exercises,
the belt squat at 145 kg, and the reason "Monday's sets all reached 6 at 140; add 5". Had
Monday gone 5, 5 and 3, the answer would be "keep": a set under the bottom of the range
holds the load. The preview shows a change like any other revision of a committed session.
...
generate` wrote step-ups into every Thursday of the mesocycle: October 1, 8, 15, 22 and 29.
`workout adapt` hands the strength planner every strength session from today to the last day
of the mesocycle, not the coming week's. The athlete does the leg press in the step-ups'
place on October 1. On October 2 the strength planner is asked about all four Thursdays left
and answers "keep" for each: once is not a habit (above). The athlete does the leg press
again on October 8. On October 9 the strength planner is asked about October 15, 22 and 29,
sees the swap twice, and returns the three sessions with the leg press in the step-ups'
place and the reason "You did the leg press in place of the step-ups twice, so I wrote it
in." So a habit reaches the rest of the mesocycle in one morning.

The check of a kept session has three grounds for a change. The sets on record say the load
should move. The athlete has done something else than what is written as many times as make
a habit. Or the brief or the duration is not the one the sets were written under. The third
is not new to the code. The evidence rule below has applied such a change since phase 2, and
its example is the Thursday cut from 70 to 40 minutes. It was missing from the prompt.
Revision 17 told the strength planner to keep a session unless the load should move, and
never told it that a brief was new, so it answered "keep" to a change code would have
applied. Now a session to check whose brief or duration moved carries a line that says so,
and the TASK says to write that session again, from the most comparable session as done,
like a session to write. It stays a session to check all the same, so a call that fails
twice keeps its sets and does not fail the proposal.

That is what a `workout generate` over days that already hold sessions does. It is Sunday
September 20, and the athlete runs `workout generate -g 4`. The week planner writes every
session of the goal again, the briefs with them. A strength session that lands on a date
where one already stands is the same session (below), so it arrives as a session to check
with October 5's five exercises. Its brief is new, so the strength planner writes it again,
and the preview shows the twelve exercises before the athlete accepts them. This is also how
the 24 sessions revision 17 wrote get the new shape (§11.3), and how the sessions follow a
change to the athlete's science file. Inside the committed days
(DESIGN_plan_change_continuity.md §4) the week planner is asked to keep a session rather
than write it again, a kept session keeps its brief, and its sets stand. `replan` runs `workout
...
Alternation lives in the notes, and notes alone never change a session (below). So a kept
session whose exercises and order stand keeps the pairs its notes name, whatever the athlete
paired since.
...
its sets and its `strength_checks` row. Every way of moving a session does that: a
`replace` in `workout generate`, and — since main taught `workout adapt` to name where a
session came from — a move in `workout adapt` or `workout tweak`. A week planner that
carries Thursday's gym to Friday for the rain returns one entry on Friday saying it
replaces Thursday, and Friday opens with Thursday's kilograms rather than being written
from scratch. The day it left is not checked at all: the session standing there is the one
that moved away.
...
CREATE TABLE strength_checks (
    ...
    checked_against  TEXT NOT NULL          -- the stamp when last written or checked
);
```
...
`WorkoutChange.restore`, which rollback and goal reinstate use to bring back an earlier
revision, copies the sets of the revision it restores. It leaves the session's
`strength_checks` row alone, so the restored kilograms stand until new lifting is on record:
an athlete who brings back Sunday's 140 over Tuesday's 145 does not find 145 again on
Wednesday morning, from lifting already weighed. A move, which appends a copy of the session
on another date, copies the sets with it, or the moved session would carry kilograms in its
text with no rows behind them. A void has none. The cascade is there for the wipes, the
...
kettlebells offers) in the session after every working set reached the top of the range
with the reserve intact. The working sets are the entry at the heaviest load; the lighter
entries of the same exercise are its warm-up, which the rule does not read and which the
strength planner writes again around the working load. Otherwise the load holds: a working
set under the bottom of the range holds it, and a second such day in a row takes one step
off. A new load always comes with the same
...
for n + r reps: a 4–6 range with 3 in reserve is written at a 9-rep load. A new rep range
starts from the heaviest recent set in that range; when there is none, from a recent set
converted with the Epley formula (load × (1 + reps/30)), with a tenth taken off, and the
reps in reserve the session asked for added to the reps done before converting, since a set
of 5 at 140 stopped with 3 in reserve was an 8-rep set. The activity's RPE is the one
reading of how hard a day was: an activity at RPE 8 where 3 in reserve were asked was harder
than asked, and the next one holds the load.
...
can carry prescribed sets, written with it and copied by a restore and a move.
...
2. The sessions as done in `history.py` (§8): the head line, the lines joined by
   overlapping runs, and the "(not prescribed)" mark. The head line needs two things the
   history drops today: the unnamed sets, which `_activities_by_day` leaves out, and the
   activity's length, which `strength_set_rows` does not select. Tests on the join: a pair,
...
3. The strength planner's prompt in `planner_prompt.py` (§9). The sentence "Nothing else decides
   ...
   count a difference from the prescription as a habit from the `habit_after`-th time. One
...
- Two queue kinds, neither asked on the spot, both naming the activity by its day and start
  time. `sets_final`, one per activity with unnamed sets or guesses, subject the activity,
  "no, leave it" freezing as read, "Not now" meaning "I'll fix them in Connect".
  `set_names`, one per group at the freeze, a group being consecutive unnamed sets at one
  load, subject the activity, the freeze time and the positions; answers the exercises a
  person named recently, fixed when queued, plus a typed "Something else…" confirmed through
...
- The athlete's habits decide a session's content. It is written from the most comparable
  ...
  marks code made. On a day with two activities the marks go to the one that holds the most
  prescribed exercises. Nothing detects or stores a habit, and nothing defines an accessory
  or a light activity: its length, sets and RPE are on its head line. Alternated exercises
  are said in the notes, and `prescribed_sets` has no column for them. Alternating keeps the
  rests the athlete's science file asks for.
- A session to check whose brief or duration moved is written again, and the strength
  planner is told which ones those are. So a `workout generate` over days that already hold
  sessions rewrites their exercises, outside the committed days, and needs no option for it.
...
- The strength planner's exercises land in `prescribed_sets`, one row per group of sets with
  a rep range, keyed by the revision and copied by a restore and a move. The description is
  rendered from them, brief above the seam and session below. An unknown name drops that
  exercise, not the session. A call that fails twice fails the proposal when a session was
  to be written; when sessions were only to be checked they are kept, the week planner's
  changes apply, and the preview and the morning briefing say the kilograms were not
  rechecked. A session's sets change only when the returned sets differ and only on new
  ...
  lineage: a session moved to another date keeps them.
...
- Whether the strength planner counts to two reliably. It reads the not-done lines and the
  ...
  count moves into code. The marks are already made per day, so that is a small step. The
  check reaches every strength session left in the mesocycle, so a wrong count changes all of
  them on one morning.
...
- A question instead of `strength discard`. An athlete who only uses the chat bot has no way
  ...
  activities depart from what was prescribed.
- An unnamed circuit asks one question per set (§7). If an athlete who does not fix names in
  Connect lifts in circuits, asking once per load across the activity is the change, at the
  cost of a wrong name whenever two machines share a weight.
- The naming questions arrive one walk after "yes, final", because a walk covers only what
  was waiting when it started. If that morning turns out to matter, the queue could let an
  answer add items to the running walk; that is a queue change.
...
- The strength test. When an athlete has a strength goal, a mesocycle's boundary week has to
  ...
  can read. Rev. 6 §8.2 in the branch history has a rule ("a mesocycle's test measures the
...
- Pushing the prescription to the watch (`upload_workout` + `schedule_workout`) would make
  the watch name the exercises itself, which removes §7 for an athlete who follows the plan
  exactly and adds friction for one who improvises. Garmin's strength-workout payload is the
  least documented corner of the API; verify the format before promising it.
