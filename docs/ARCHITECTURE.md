...
| `stale` | `pushed_signature != ` current hash |
  ...
  - **Stale is a retry marker, not a defect.** It persists only when the push could not
    ...
    to **today onward**, so `warn_stale_before` (in `cli/workouts/calendar_sync.py`) reports
