...
`engine/adapt.py` ↔ `service/adapt.py` ↔ the adapt half of that same CLI file. The engine
reached those names first and the service carries them now too; the CLI still has no file
of its own for adapt.
