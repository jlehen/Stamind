...
from stamind import journal, runtime
from stamind import benchmarks, llm_models
from stamind.plan_versions import diff_plans, resolve_versions
from stamind.analytics import timeline
from stamind.analytics import intensity
from stamind.analytics.load import activity_load, rpe_divergence
from stamind.analytics.adherence import classify_adherence, unplanned_kind
from stamind.analytics.compare import adherence_verdicts, adherence_window, compare_days
from stamind.workout_state import calendar_status, modification_markers
from stamind.config import config
from stamind.plan_inputs import plan_config_hash
...
def compare_workouts() -> Any:
    ...
    stamind/cli/workouts/compare.py). Unlike the CLI it never calls `garmin.ensure_data` —
...
def plan_diff_versions() -> Any:
    ...
    in stamind/plan_versions.py — the CLI renders the very same structure as text."""
    goal_id = _resolve_goal_id(request.args.get("goal_id"))
    if goal_id is None:
