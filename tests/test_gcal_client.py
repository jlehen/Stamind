"""Putting a workout on the calendar and reading signal events back off it.

The push, the signal ingest and its token expiry, the event listing, and the per-event
print. Most tests here patch `.service` on this module's own `CalendarSyncer`, so the
real Google connection is never used; what the event *says* is asserted through the body
the mocked API was handed (`gcal/event.py` builds it).
"""
import os
import unittest
