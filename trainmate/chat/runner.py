...
A chat message is a TrainMate command line, and the bot answers it by running
`trainmate_cli.py` as a child process. `Session` is that child plus the state needed to
...
The child is launched with `TRAINMATE_FRONTEND=json`, so a `confirm`/`choose`/`text`
...
from trainmate.chat.routing import ROUTER_TIMEOUT_SECONDS
