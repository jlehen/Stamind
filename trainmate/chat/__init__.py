...
`tm bot route` can read the router's intent table without a chat front-end appearing on a
command line.

This file stays a docstring on purpose. Importing `trainmate.chat.routing` runs it, so
assembling `ChatBot` here would put the whole front-end on the path of every `tm bot
