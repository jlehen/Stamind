...
``from trainmate.coach import honoring`` costs the one module and not the whole coach
(AGENTS.md, Code style):
  ...
  - ``engine``     — ``CoachEngine``: prompt construction, hashing, and LLM calls. Holds
                     ...
                     call is ``trainmate.coach.engine.openrouter_client``.
...
The ``config`` singleton is ``trainmate.config.config``, the one object every submodule
imports, so an in-place mutation of ``trainmate.config.config.data`` is visible
everywhere.
"""
