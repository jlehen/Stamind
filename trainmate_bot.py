"""TrainMate Telegram front-end — the launcher.
...
chat, state in ``trainmate.chat.runner.Session``, a per-prompt ``nonce`` against stale
taps.

This file is only the entry point ``tm-bot`` runs. The bot itself is ``ChatBot`` in
``trainmate/chat/app.py``, and the rest of ``trainmate/chat/`` holds what one message
...
``trainmate.chat.runner.RESTART_EXIT_CODE``, which is what ``/restart`` exits with.
