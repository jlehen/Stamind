...
from stamind.cli.workouts.adapt import run_workout_adapt, run_workout_tweak
from stamind.cli.workouts.compare import run_workout_compare
from stamind.cli.workouts.generate import run_workout_generate
...
def _history_depth(value: str) -> int:
    """`workout show --history DEPTH`: a count, where 0 or less and 'all' mean every one."""
    if value == "all":
        return 0
    try:
        return int(value)
...
def add_workout_parser(subparsers, pull_bypass_parser, llm_debug_parser):
    ...
    w_show.add_argument(
        ...
        help="Also show the earlier forms each session had, newest first: when it was "
             ...
             "DEPTH is how many earlier forms to show at most; no DEPTH, 'all', 0 or less "
             "shows every one"
    )
    ...
    w_prune = workout_subparsers.add_parser(
        ...
        description=(
            "Sweep the Google Calendar for Stamind workout events that no workout in "
