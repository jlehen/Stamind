...
from stamind.analytics.baselines import classify_metric, is_anomalous, UNKNOWN
...
from stamind.clock import fmt_date, today_date as _today_date, today_str as _today_str
from stamind.cli.common import constraint_line, ensure_recent_data
...
def run_status(args) -> None:
    ...
    print(bold(cyan("=== STAMIND ATHLETE STATUS ===")))
    ...
    if objectives:
        ...
        print(
            f"\n{bold('Next Goal')}: None (Stamind needs at least one goal to start planning)"
        )
    ...
    from stamind.benchmarks import ANCHOR_KINDS, LOGBOOK_KINDS, format_value
    threshold_lines = []
