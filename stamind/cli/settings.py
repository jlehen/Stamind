...
See DESIGN_settings.md §4. `stamind/settings.py` owns what a setting is and how it
...
from stamind import clock, llm_models, settings
from stamind.text import bold, cmd, cyan, dim, green, pad_visible, red, visible_len, yellow
...
def run_settings_set(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        notice(str(e), red)
        sys.exit(1)
    # Through the renderer: `bot capture change_setting` lands here, and its confirm has
    # already read the change back as its effect (§12.7).
    from stamind import runtime
    runtime.render.setting_changed(setting.name, stored, before)
    if stored == before:
        return
