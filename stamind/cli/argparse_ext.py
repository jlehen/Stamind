...
Extracted from stamind_cli.py; those names are re-imported there so existing
``stamind_cli.<name>`` patch seams keep working."""
import argparse
import re
import sys
import textwrap
...
def translate_dashless_argv(parser: argparse.ArgumentParser, tokens: list) -> list:
    ...
    deliberate rather than mechanical:
      ...
      * a sub-command is emitted as its canonical name before the remainder is translated
        in that sub-parser's context, so the dispatcher in ``stamind_cli`` compares
    ...
    while i < n:
        ...
        if sub_action is not None:
            ...
            if fallback is not None and i == 0:
                ...
                # are the values of the dashed options passed through above.
                out.extend(descend(fallback, tokens[i:]))
                return out
        out.append(tok)
        i += 1
    return out
