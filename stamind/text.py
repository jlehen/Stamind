...
here knows what a date is. The only app module it reaches for is `stamind.sentinels`,
...
def display_width() -> int:
    ...
    explicit STAMIND_WRAP_WIDTH still wins, because that is a narrow client stating its
    ...
    if os.environ.get("STAMIND_WRAP_WIDTH") or not sys.stdout.isatty():
...
def asides_enabled() -> bool:
    ...
    it as history above the answer. STAMIND_VERBOSE=1/0 forces either way — no CLI
    flag, `-v` is taken (DESIGN_output_verbosity.md §2/§4)."""
    raw = os.environ.get("STAMIND_VERBOSE")
    if raw:
        return raw.lower() not in ("0", "no", "false")
    from stamind.sentinels import is_json_frontend
