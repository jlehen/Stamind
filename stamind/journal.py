...
def _report_write_failure(exc: Exception) -> None:
    ...
    try:
        from stamind.text import asides_enabled
        if asides_enabled():
...
def start_run(
    ...
    run = Run(
        ...
        argv=argv,
        source=source or os.environ.get("STAMIND_SOURCE") or "cli",
        parent=parent or current_id() or os.environ.get("STAMIND_PARENT_RUN") or None,
        started=time.monotonic(),
    )
    _stack.append(run)
    fields = dict(
        argv=argv, source=run.source, pid=os.getpid(),
        frontend=os.environ.get("STAMIND_FRONTEND") or "tty",
...
def child_env(env: Dict[str, str], source: str) -> Dict[str, str]:
    ...
    run_id = current_id()
    if run_id:
        env["STAMIND_PARENT_RUN"] = run_id
    else:
        env.pop("STAMIND_PARENT_RUN", None)
    env["STAMIND_SOURCE"] = source
    return env
