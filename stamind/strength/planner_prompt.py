...
def _shipped_science() -> str:
    ...
    return "\n".join([
        ...
        "Stamind's own progression rules, shipped with the app. The athlete's own "
...
def clean_session(raw: Dict[str, Any], dropped: List[str]) -> Answer:
    ...
    for entry in raw.get("exercises") or []:
        ...
        if cleaned is None:
            ...
            dropped.append(
                f"{raw.get('date')}: '{name}' is not an exercise Stamind knows, left out"
            )
            continue
        rows.append(cleaned)
    if raw.get("light"):
