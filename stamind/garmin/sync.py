...
from stamind.analytics.load import compute_load, measured_tss
from stamind.analytics.pmc import derivation_pad_days
from stamind.clock import date_range, parse_date, shift, today_str
from stamind.config import config
from stamind.text import cmd, keep_whole
from stamind.output import Progress, fail, step, warn
import stamind.garmin as _g
...
from stamind.strength import sets as strength_sets
...
def _sync_calendar_signals(force: bool) -> None:
    ...
    try:
        from stamind.gcal import client as gcal_client
    except Exception:
...
def _pull_command(start: str, end: str) -> str:
    return f"python stamind_cli.py data pull -d {start}..{end}"
...
def ensure_data(start_date: str, end_date: str, force: bool = False) -> None:
    ...
    # The window is the shared one; `stamind/freshness.py` says why it is not two rules.
    ...
    for region in auto_regions:
        try:
            ...
            warn(
                ...
                + cmd("python stamind_cli.py data pull")
