...
# Which config file this process runs against; STAMIND_CONFIG is how a second athlete
...
CONFIG_PATH = os.path.abspath(
    os.path.expanduser(os.environ.get("STAMIND_CONFIG") or _DEFAULT_CONFIG_PATH))
...
class Config:
    ...
    def __init__(self) -> None:
        ...
        A file named via STAMIND_CONFIG must exist and parse: the env var picks which
        ...
        explicit = bool(os.environ.get("STAMIND_CONFIG"))
        if not os.path.exists(CONFIG_PATH):
            if explicit:
                raise SystemExit(f"STAMIND_CONFIG names a missing file: {CONFIG_PATH}")
            return
        try:
            ...
            if explicit:
                raise SystemExit(f"STAMIND_CONFIG file failed to load: {CONFIG_PATH} ({e})")
    ...
    def db_path(self) -> str:
        ...
        stamind.db. A relative value resolves against the instance's `data_dir`, so a
        STAMIND_CONFIG instance cannot silently open another instance's database."""
        return self._resolve(self.get("database") or "stamind.db")
    ...
    def science_dir(self) -> str:
        ...
        so a STAMIND_CONFIG instance gets its own training philosophy rather than
    ...
    def logging_dir(self) -> str:
        ...
        and `science_dir:`, so a STAMIND_CONFIG instance keeps its logs beside its own
    ...
    def telegram_bot_token(self) -> Optional[str]:
        """Telegram bot token used by the chat front-end (stamind_bot.py).
