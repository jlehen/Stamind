...
JSON object:
    ...
    \\x1eSM-PROMPT  {"v": 1, "id": …, "type": "confirm", "message": …}   answer expected
    \\x1eSM-PHOTO   {"path": …, "caption": …}                            one-way
...
BUTTONS_SENTINEL = "\x1eSM-BUTTONS "
...
def is_json_frontend(frontend: Optional[str] = None) -> bool:
    ...
    `STAMIND_FRONTEND=json` is set by the bot on the commands it spawns. Off a terminal
    but without it — a cron run, a piped command — the answer is False: nothing is there
    to read a frame."""
    if frontend is None:
        frontend = os.environ.get("STAMIND_FRONTEND", "")
...
def parse_frame(line: str) -> Optional[Tuple[str, Any]]:
    ...
    SM-FLUSH payload is legitimately ``{}``, and a bare falsy dict reads as "not a frame"
    at every call site.
...
def prompt_answer(
    ...
    """The reply to a SM-PROMPT, as the CLI's `JsonPrompt` expects to read it back.
